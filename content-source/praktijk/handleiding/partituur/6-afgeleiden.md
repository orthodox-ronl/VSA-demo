...
Hub: `{stam}.pdf` en `{stam}.mxl`. VSA: `{stam}.vsa.mxl`. De bron blijft de
plek waar je editet (hub-`.mscz` of `.vsa`).

**Wanneer:** na elke inhoudelijke wijziging aan een hub-`.mscz` of
bibliotheek-`.vsa` die al gepubliceerd wordt, vóór je opnieuw
`publicatiestatus: reviewable` (of hoger) zet. Eerste keer hub-producten:
[PDF en Coria](../5-pdf-en-coria/). Eerste keer VSA-Coria:
[.vsa schrijven](../../vsa/1-vsa-schrijven/).

## Hub: volgorde (niet omdraaien)
...
de preview een rode banner dat de hub-afgeleiden niet bij de hub-hash horen.
...
site. Zie [Publicatietrajecten](../../start/publicatietrajecten/).
...
| --- | --- | --- |
| Preview-banner: hub-afgeleiden niet in orde | PDF/MXL niet vernieuwd na hub-edit | Hub-stappen 2–4 opnieuw |
...
`check --strict` is groen; hub-PDF/MXL horen bij de huidige hub; VSA-`.vsa.mxl`
