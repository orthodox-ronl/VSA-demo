...
Basispartituur-layout/normalisatie: [mscz-partituur-contract.md](mscz-partituur-contract.md).
Basispartituur-transforms: [mscz-product-transforms.md](mscz-product-transforms.md).
...
| -------- | ---------------------------------------- |
| Sibling basispartituur-`.mscz` aanwezig | impliciet representatie `partituur` |
| Alleen `.vsa` (geen basispartituur-`.mscz`) | impliciet `vsa` (zodra VSA-productgate er is) |
| Sibling `{stam}.print.mscz`, geen basispartituur | PDF hoort bij `print`; korte `{stam}.pdf` mag |
...
**Principe** (geldt voor basispartituur, vsa, print en latere sporen zoals mvsa): elk
...
| ----- | ----------------- | ---- |
| `partituur` | Colofon + meta in basispartituur-`.mscz` → PDF via MuseScore-export | `ensure_bibliotheek_id` / `check_bibliotheek_id` |
...
- Partituur-productgate en VSA-productgate **slaan** zulke mappen over.
...
basispartituur-producten) slaan die mappen over. `python scripts/bibliotheek.py` (in
...
eerst (basispartituur als er een basispartituur-`.mscz` is, anders vsa) en een korte hulpzin in
...
| Situatie | BPM |
| -------- | --- |
| Expliciet in basispartituur-metronoom of `.vsa`-frontmatter `tempo:` | die waarde |
...
- Basispartituur-afgeleiden: handleiding `partituur/5-pdf-en-coria`, `6-afgeleiden`
