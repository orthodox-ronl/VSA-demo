"""Controleer hub-.mscz vs PDF/Coria-MXL via embedded hub-sha256.

Schrijft data/hub-product-status.json voor Hugo-banners.
...
STATUS_PATH = REPO_ROOT / "data" / "hub-product-status.json"
...
def check_one(mscz: Path) -> FolderStatus:
    ...
    if not pdf.is_file():
        issues.append(
            Issue("missing_pdf", _rel(pdf), "PDF ontbreekt naast de hub-.mscz")
        )
    else:
        stamp = read_pdf_stamp(pdf)
        got = stamp.get(FIELD_HUB_SHA, "")
        if not got:
            issues.append(
                Issue(
                    "unstamped_pdf",
                    _rel(pdf),
                    "PDF heeft geen VSAHubSHA256-metadata (opnieuw genereren)",
                )
            )
        elif got != hub_hash:
            ...
            issues.append(
                Issue(
                    "stale_pdf",
                    _rel(pdf),
                    f"PDF-hub-hash wijkt af (gegenereerd {when}); hub is gewijzigd",
                )
            )

    if not mxl.is_file():
        issues.append(
            Issue("missing_mxl", _rel(mxl), "Coria-.mxl ontbreekt naast de hub-.mscz")
        )
    else:
        ...
        if not got:
            issues.append(
                Issue(
                    "unstamped_mxl",
                    _rel(mxl),
                    "MXL heeft geen vsa-hub-sha256 (opnieuw genereren)",
                )
            )
        elif got != hub_hash:
            when = stamp.get(FIELD_GENERATED_AT, "?")
            issues.append(
                Issue(
                    "stale_mxl",
                    _rel(mxl),
                    f"MXL-hub-hash wijkt af (gegenereerd {when}); hub is gewijzigd",
                )
            )
...
def main() -> int:
    ...
    print(
        f"Hub-producten: {len(statuses) - len(bad)} ok, {len(bad)} probleem "
